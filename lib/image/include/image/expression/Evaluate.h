...
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Expr &expr, Coord... coords) noexcept {
    return expr(coords...);
}

/// If expression is not invocable with the provided coordinates, then return it as-is.
template <typename Expr, typename... Coord, std::enable_if_t<!std::is_invocable_v<Expr, Coord...>, bool> = true>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Expr &expr, [[maybe_unused]] Coord... coords) noexcept {
    return expr;
}

/// When omitting the plane number, invoke the image at plane 0.
template <typename T>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Image<T> &image, int x, int y) noexcept {
...
template <typename T>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Image<T> &image, int x, int y, int n) noexcept {
    return image(x, y, image.numPlanes() > 1 ? n : 0);
}

/// When omitting the plane number, invoke the image at plane 0.
template <typename T>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const ImageView<T> &imageView, int x, int y) noexcept {
...
/// Ensure we always invoke a one plane image at plane 0.
template <typename T>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const ImageView<T> &imageView, int x, int y, int n) noexcept {
    return imageView(x, y, imageView.numPlanes() > 1 ? n : 0);
}

/// Invoke a plane at (x, y) coordinates whatever the plane number n.
template <typename T>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const PlaneView<T> &planeView,
                                           int x,
                                           int y,
...
template <typename T, int N>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Pixel<T, N> &pixel,
                                           [[maybe_unused]] int x,
                                           [[maybe_unused]] int y,
                                           int n) noexcept {
    return pixel[n];
...
template <int M, int N>
UTIL_ALWAYS_INLINE decltype(auto) evaluate(const Matrix<M, N> &matrix,
                                           [[maybe_unused]] int x,
